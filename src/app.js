// AIRI Voice Chat — Main Application
const VOICES={
...
let ws=null; // WebSocket to Cloudflare WSS
...
function computeVisibleBounds(){
  ...
  if(!mdl||!app)return;
  try{
    // Method 1: Render to offscreen canvas and scan for non-transparent pixels
    const rt=PIXI.RenderTexture.create({width:256,height:Math.round(256*origH/origW)});
    const prevX=mdl.x,prevY=mdl.y,prevSX=mdl.scale.x,prevSY=mdl.scale.y;
    const sc=256/origW;
    mdl.scale.set(sc);mdl.x=128;mdl.y=rt.height/2;
    app.renderer.render(mdl,{renderTexture:rt});
    mdl.x=prevX;mdl.y=prevY;mdl.scale.set(prevSX,prevSY);
    const pixels=app.renderer.plugins.extract.pixels(rt);
    const tw=rt.width,th=rt.height;
    rt.destroy(true);
    let minX=tw,minY=th,maxX=0,maxY=0,found=false;
    for(let y=0;y<th;y++){
      for(let x=0;x<tw;x++){
        if(pixels[(y*tw+x)*4+3]>10){
    ...
    if(found&&(maxX-minX)>2&&(maxY-minY)>2){
      const cx=((minX+maxX)/2-128)/sc;
      const cy=((minY+maxY)/2-th/2)/sc;
      visBounds={x:cx,y:cy,w:(maxX-minX)/sc,h:(maxY-minY)/sc};
    }
...
async function initL2D(){
  ...
  // ── Touch/Mouse: dead zone + pet detection ──
...
// ── Send: WSS first, fallback HTTP bridge ──
...
window.draw=(type)=>{
  ...
  if(type==='voice'){
    ...
    let html='<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px"><button onclick="window.resetParams()" style="padding:8px 16px;background:#ef4444;border:none;border-radius:7px;color:white;cursor:pointer;font-size:.85em">Reset All</button></div>';
    html+='<div class="f"><label style="color:#c084fc;font-weight:600;">Texture Atlas</label><p style="font-size:.72em;color:#888;margin:4px 0 8px;">Live2D models use texture atlases (PNG sheets with all body parts combined). To change the avatar appearance: 1) Export the current texture, 2) Edit in an image editor (keep same layout), 3) Replace below.</p>';
    try{
      const texCount=mdl.internalModel.textureCount||mdl.internalModel.getTextureCount?.()
        ||(mdl.internalModel.textures?mdl.internalModel.textures.length:1);
      for(let ti=0;ti<(texCount||1);ti++){
        html+='<label style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;background:#1a1a3a;border:1px solid #2a2a4a;border-radius:7px;cursor:pointer;font-size:.78em;color:#c084fc;margin:2px"><input type="file" accept="image/png,image/jpeg" onchange="window.swapTexture(this.files[0],'+ti+')" style="display:none">Replace Texture '+ti+'</label>';
      }
    }catch{html+='<label style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;background:#1a1a3a;border:1px solid #2a2a4a;border-radius:7px;cursor:pointer;font-size:.78em;color:#c084fc;margin:2px"><input type="file" accept="image/png,image/jpeg" onchange="window.swapTexture(this.files[0],0)" style="display:none">Replace Texture</label>'}
    html+='</div>';
...
window.swapTexture=(file,idx)=>{
  ...
  img.onload=()=>{
    ...
    mdl.internalModel.bindTexture(idx,tex);
    addMsg('s','Texture '+idx+' replaced');
  };
...
// ── Auto-connect: WSS first on mobile, HTTP bridge on desktop ──
(async()=>{
  const isMobile=/Android|iPhone|iPad/i.test(navigator.userAgent);
  if(isMobile){wssConnect();return}
  const tryUrls=[bridge,'http://localhost:3456'];
